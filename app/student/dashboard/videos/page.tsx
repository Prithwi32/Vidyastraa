...
//   MATHEMATICS: { id: "PLgUwDviBIf0rqmgarU5Rx6MP6Qt1_uVx4", name: "Mathematics" },
...
//   const filteredVideos = Object.entries(videos).reduce((acc, [subject, videoList]) => {
...
//           <h2 className="text-2xl sm:text-3xl font-bold tracking-tight mb-1">🎓 Video Lectures</h2>
...
//                         <h3 className="font-semibold text-xs sm:text-sm line-clamp-2">{video.title}</h3>
//                         <p className="text-xs text-muted-foreground line-clamp-1">
//                           {video.instructor} • {video.views} • {video.uploadedDate}
//                         </p>
//                       </CardContent>
//                     </Card>
...
//               {searchQuery ? "Try a different search term" : "No videos available for this category"}
//             </p>
//           </div>
//         )}
//       </Tabs>
...
const PLAYLISTS: Record<string, { id: string; name: string }> = {
  PHYSICS: { id: process.env.NEXT_PUBLIC_PHYSICS_PLAYLIST_ID || "PLgUwDviBIf0oF6QL8m22w1hIDC1vJ_BHz", name: "Physics" },
  CHEMISTRY: {
    id: process.env.NEXT_PUBLIC_CHEMISTRY_PLAYLIST_ID || "PLgUwDviBIf0pMFMWuuvDNMAkoQFi-h0ZF",
    name: "Chemistry",
  },
  MATHEMATICS: {
    id: process.env.NEXT_PUBLIC_MATHEMATICS_PLAYLIST_ID || "PLgUwDviBIf0rqmgarU5Rx6MP6Qt1_uVx4",
    name: "Mathematics",
  },
  BIOLOGY: { id: process.env.NEXT_PUBLIC_BIOLOGY_PLAYLIST_ID || "PLgUwDviBIf0pMFMWuuvDNMAkoQFi-h0ZF", name: "Biology" },
  JEE: { id: process.env.NEXT_PUBLIC_JEE_PLAYLIST_ID || "PLgUwDviBIf0oF6QL8m22w1hIDC1vJ_BHz", name: "JEE Preparation" },
  NEET: {
    id: process.env.NEXT_PUBLIC_NEET_PLAYLIST_ID || "PL9gnSGHSqcnrslTujkMYzx-GuVrpVpu5_",
...
const formatViews = (views: string | number): string => {
  const num = typeof views === "string" ? Number.parseInt(views) : views
  if (isNaN(num)) return "N/A views"
...
export default function VideosPage() {
  ...
  return (
    <div className="min-h-screen p-3 sm:p-4 md:p-8 max-w-7xl mx-auto overflow-hidden">
      <div className="mb-6 md:mb-8 flex flex-col gap-4">
        <div className="text-center md:text-left">
          <h2 className="text-2xl sm:text-3xl font-bold tracking-tight mb-1">🎓 Video Lectures</h2>
          <p className="text-muted-foreground text-sm sm:text-base">
      ...
      <Tabs defaultValue={Object.keys(PLAYLISTS)[0]} className="space-y-6">
        <div className="overflow-x-auto pb-2 -mx-3 px-3 sm:mx-0 sm:px-0">
          <TabsList className="w-max md:w-full justify-start rounded-md border bg-background p-1 shadow-sm">
            {Object.entries(PLAYLISTS).map(([key, { name }]) => (
              <TabsTrigger key={key} value={key} className="text-xs sm:text-sm">
                {name}
              </TabsTrigger>
        ...
        {loading ? (
          <div className="grid gap-3 sm:gap-4 md:gap-6 grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4">
            {[...Array(8)].map((_, i) => (
              <Card key={i} className="overflow-hidden">
                <Skeleton className="aspect-video w-full" />
          ...
          Object.entries(filteredVideos).map(([subject, videoList]) => (
            <TabsContent key={subject} value={subject}>
              <div className="grid gap-3 sm:gap-4 md:gap-6 grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 w-full">
                {videoList.map((video) => (
                  <Dialog key={video.id}>
                    <Card className="overflow-hidden shadow-sm hover:shadow-md transition-shadow border h-full flex flex-col">
                      <DialogTrigger asChild>
                        <div className="relative aspect-video group cursor-pointer flex-grow-0">
                          <Image
                            ...
                            className="object-cover"
                            sizes="(max-width: 640px) 100vw, (max-width: 768px) 50vw, (max-width: 1024px) 33vw, 25vw"
                            priority={videoList.indexOf(video) < 4}
                          />
                          <div className="absolute inset-0 bg-black/30 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                      ...
                      <CardContent className="p-2 sm:p-3 md:p-4 space-y-1 flex-grow">
                        <h3 className="font-semibold text-xs sm:text-sm line-clamp-2">{video.title}</h3>
                        <p className="text-xs text-muted-foreground line-clamp-1">
                    ...
                    <DialogContent className="sm:max-w-[90vw] md:max-w-[80vw] lg:max-w-[70vw] max-h-[90vh] p-0 overflow-hidden w-[calc(100vw-32px)]">
                      <div className="aspect-video w-full">
                        <iframe
                          className="w-full h-full"
                          src={`https://www.youtube.com/embed/${video.id}?autoplay=1`}
                      ...
                      <div className="p-3 sm:p-4 overflow-y-auto max-h-[30vh] sm:max-h-[40vh]">
                        <h3 className="font-semibold text-sm sm:text-base md:text-lg">{video.title}</h3>
                        <p className="text-xs sm:text-sm text-muted-foreground mt-1">
                          {video.instructor} • {video.views} • {video.uploadedDate}
                        </p>
                        <p className="text-xs sm:text-sm mt-2 whitespace-pre-line line-clamp-6 sm:line-clamp-none">
                          {video.description}
                        </p>
                      </div>
                    </DialogContent>
